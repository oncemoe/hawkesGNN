...
class LiveLinkPrediction(LinkPrediction):
    ...
    def train(self, args, model, dataset, split):
        ...
        for t in range(task_range):
            ...
            val_loss, mets, _cnt = self.test(model, dataset[t], args.device, args.row_mrr)
            ...
            lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs, eta_min=1e-5, last_epoch=-1)
            ...
            for epoch in range(1, 1 + args.epochs):
                ...
                train_loss = self.train_epoch(model, dataset[t], args.device, optimizer)
                t1 = time.perf_counter()
                lr_scheduler.step()

                t2 = time.perf_counter()
                val_loss, mets, _ = self.test(model, dataset[t], args.device, False)
                mrr = mets[1]
    ...
    def main(self, args, factory, ds_split):
        ...
        save_result(f"{args.model})_live_update_{args.roland_updater}_{args.dataset}{is_meta}.log", args, mrr_lists, gpu_usage_list, time_usage_list, not args.no_log)
